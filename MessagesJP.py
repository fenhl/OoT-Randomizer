...
NAVI_MESSAGES = list( range(0x0100, 0x01AB))
for id in REJECT:
    if id in NAVI_MESSAGES:
        NAVI_MESSAGES.remove(id)
NAVI_MESSAGES_Z = list( range(0x0600, 0x065C))
for id in REJECT:
    if id in NAVI_MESSAGES_Z:
        NAVI_MESSAGES_Z.remove(id)
MASK_MESSAGES = list( range(0x7100, 0x711F) )
MASK_MESSAGES += list( range(0x7124, 0x71AC) )
ITEM_MESSAGES = {
    0x0001: "<<+T\x30##\x01テキストエラー！##\x00",
    0x9001: "<<~~\x2D##\x01タマゴ##\x00をお借りした。",
    ...
    0x9048: "<<~~\x10##\x01マメ袋\x00を入手！&&いい場所さがして　まこう！",
...
MISC_MESSAGES = {
    0x0205: ("<<##\x01ダンペイさん##\x00は&&おネムの　時間だ！", 0x22),
    0x0206: ("<<##\x01ダンペイさん##\x00は&&おネムの　時間だ！", 0x22),
    0x0207: ("<<##\x04しあわせのお面屋##\x00&&ぜひ　ご利用下さい。　　　店主", 0x22),
    0x0227: ("<<だれ！？&&ウラぐちから　入ろうとしてる&&ワルい子は！", 0x02),
    0x022C: ("<<この地に眠る魂&&ハイラル王家に&&忠誠を誓いし者の魂なり", 0x22),
    0x0311: ("<<命知らずな方は　ぜひとも&&当店に　お立ち寄りください&&じゅうたん商人", 0x10),
    0x0329: ("<<##\x04的当て屋　　１回　２０ルピー##\x00", 0x13),
    0x0337: ("<<##\x05Ｚの穴##\x00", 0x10),
    0x0340: ("<<注目する相手が　いない時は&&Ｚで　前を見ることができます", 0x10),
    0x0341: ("<<飛びたい方向に　おもいきって&&走れば　勝手に飛べます", 0x10),
    0x0343: ("<<##\x01となりの石##\x00は　注目できます", 0x10),
    0x0344: ("<<##\x02森のステージ##\x00&&ごうか賞品アリ", 0x10),
    0x1007: ("<<オイラと　れんしゅうしようゼ～！", 0x00),
    0x100A: ("<<オイラ、サリアんちの&&草かり　やらされてんだヨ。&&ミドのアニキの　命令なんだ～っ！}}\x10\x0B", 0x00),
    ...
    0x2085: ("<<も一度　ちょ～せんするだーか？^^<<１回　５ルピーで　やる？>>&&:2##\x02はい&&いいえ##\x00", 0x00),
    0x2086: ("<<マロンのムコさんにならね～だか？>>&&:2##\x02はい&&いいえ##\x00", 0x00),
    0x2087: ("<<いや～　じょーだん　じょーだん！&&約束どおり##\x01イイもの##\x00やるだーよ！{{", 0x00),
    0x2088: ("<<おっきくなったら　この牧場で&&はたらかねえだか？&&いつでも　待ってるだーよ。{{", 0x00),
    0x2089: ("<<ほら、とれとれの&&##\x01ロンロン牛乳##\x00　持って行くだ！{{", 0x00),
    0x208A: ("<<ざんねんだーよ。&&からのビンが　ないだーよ。", 0x00),
    0x208B: (":3##\x02ロンロン牛乳　　　３０ルピー&&コッコ当てゲーム　１０ルピー&&やめる##\x00", 0x00),
    0x208E: ("<<もう一度　挑戦するなら&&エポナに乗って　準備して！", 0x00),
    0x208F: ("<<やったわネ　@N！&&##\x01@R##\x00、新記録よ！", 0x00),
    0x2090: ("<<これまでの記録は　##\x01５０秒##\x00。&&キミの記録は　##\x01@H\x03##\x00。&&じゃ、始めるヨ！{{", 0x00), 
    0x2091: ("<<記録は　キミの　##\x01@H\x03##\x00。&&じゃ、始めるヨ！{{", 0x00),
    0x2092: ("<<これまでの記録は　##\x01５０秒##\x00。&&じゃ、始めるヨ！{{", 0x00),
    0x507B: ("<<オレ、ほんとに　見たんだよ！&&墓守りダンペイが##\x01お宝##\x00を持って&&自分の墓の中に消えてくのをサ…", None),
    ...
    0x4012: ("<<余の　感謝の気持ちとして&&##\x01これ##\x00を　さずけよう！", 0x03),
    0x401B: ("<<おー、その手紙は！？&&##\x01ルト姫の手紙##\x00ゾラ！！^^<<入っていた##\x01ビン##\x00は、そちにやるから&&ありがたく　受け取るゾラ！}}\x40\x1C", 0x03),
    0x401C: ("<<はやく、余の　かわいい　##\x01ルト姫##\x00を&&見つけ出してきてくれぃ…ゾラ！$$\x68\x7A", 0x23),
    0x70F4: ("<<ニイさんも&&ポウを　つかまえてきたら&&高く　買ってやるよ。　イーッヒッヒ！", 0x00),
    0x70FC: ("<<わたし、夢を　見たのです。&&##\x02みどりに光る石##\x00を　かかげた&&人の姿に　変わったのです。{{", 0x03),
    0x7123: ("<<よかった。あなたが　来てくれて…{{", 0x03),
    0x71AF: ("<<パーフェクト～ッ！！！{{", 0x00),
    0x9100: ("<<マタ　来テネ～。", 0x00)
}
...
def update_item_messages_jp(messages):
    ...
    ntext = "<$いたいッピ！　かんべんッピ！{"
    update_message_jp(messages, 0x101A, ntext, 0x00, 1)
    ...
    for id in NAVI_MESSAGES:
        update_message_jp(messages, id, "<？", 0x03)
    for id in NAVI_MESSAGES_Z:
        update_message_jp(messages, id, "<？", 0x20)
    for id in MASK_MESSAGES:
        update_message_jp(messages, id, "<ひいいっ！！", 0x00),
