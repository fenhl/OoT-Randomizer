...
from MessagesJP import update_message_jp, read_shop_items, \
        ...
        add_item_messages, write_messages, shuffle_messages_jp, reproduce_messages_jp
...
chestTypeMap = {
    ...
    0x2000: [0x5000, 0x0000, 0x2000], #Boss Key窶冱 Chest
...
def configure_dungeon_info(rom, world):
    mq_enable = (world.settings.mq_dungeons_random or world.settings.mq_dungeons != 0 and world.mq_dungeons != 12)
...
def patch_rom_jp(spoiler:Spoiler, world:World, rom:Rom):
    ...
    # Force language to be Japanese in the event a English rom was submitted
    ...
    update_message_jp(messages, 0x70F5, new_message)
    if world.settings.big_poe_count != 10:
        ...
        update_message_jp(messages, 0x70f7, new_message)
        ...
        update_message_jp(messages, 0x70f8, new_message)
    ...
    update_message_jp(messages, 0x5036, new_message)

    # Update "Princess Ruto got the Spiritual Stone!" text before the midboss in Jabu
    reward_text = {'Kokiri Emerald':   "#\x02コキリのヒスイ#\x00",
    ...
    new_message = "]<ルト姫が%sを入手！>&+S\x02…って、+S\x00　なんで　ルト姫なの？" % reward_text[world.get_location('Barinade').item.name]
    update_message_jp(messages, 0x4050, new_message)
    ...
    if world.settings.skip_some_minigame_phases:
        ...
        update_message_jp(messages, 0x6040, "<よぉ、新入り！&いい馬に　乗ってるじゃないか！^<どうだい、#\x01やぶさめ#\x00に&挑戦してみないか？^<行きと帰りで　何ポイントとれるか。&使える矢は　２０本だ。^<#\x01１０００ポイント#\x00　獲れれば&イイものを　やろう。^<そして　#\x01１５００ポイント#\x00　獲れれば&もっとイイものを　やろう！{")
    ...
    # describe
    update_message_jp(messages, 0x80FE, '<#\x01ボムチュウ（５コ）　６０ルピー&#\x00自分で走る　新型バクダン。>*O', 0x03)
    # purchase
    update_message_jp(messages, 0x80FF, '<ボムチュウ（５コ）　６０ルピー&:2#\x02かう&やめとく#\x00>', 0x03)
    ...
    if world.settings.shuffle_scrubs == 'off':
        ...
        for (scrub_item, default_price, text_id, text_replacement) in business_scrubs:
            if scrub_item not in single_item_scrubs.keys():
                continue
            scrub_message_dict[text_id] = update_scrub_text_jp(text_id, default_price, default_price)
    else:
    ...
    for text_id, message in scrub_message_dict.items():
        update_message_jp(messages, text_id, message, mode = 1)
    ...
    if world.settings.shuffle_beans:
        ...
        update_message_jp(messages, 0x405E, "ポリ…ポリ…ポリ…&<#\x01アイテム#\x00…　あるよ…&どうだい？^<#\x01<６０ルピー#\x00なら　いいぜ！　ケケ！&:2#\x02かう&やめとく#\x00>")
        update_message_jp(messages, 0x4069, "<お金がないなら　売れねぇな。>&ポリ…ポリ…")
        update_message_jp(messages, 0x406C, "<まいどあり！>&ポリ…ポリ…ポリ…")
    ...
    if world.settings.shuffle_medigoron_carpet_salesman:
        ...
        update_message_jp(messages, 0x6077, "<+T\x1Bイ～ラッシャ～イ！！^<今回ノ　商品ハ…&オ金モラウマデ　ヒミツデ～ス！^<#\x01２００るぴぃ#\x00デ～ス。　買ウ？&:2#\x02かう&やめとく#\x00>")
        update_message_jp(messages, 0x6078, "<マイドアリ～！！&気ガ変ワッタラ　マタ　来テネ～。")
        ...
        update_message_jp(messages, 0x304C, "<スッゴイもの　もってるゴロ。&ためしに…}\x30\x4F")
        update_message_jp(messages, 0x304D, "<どう…ゴロ？")
        update_message_jp(messages, 0x304F, "<２００ルピーで、買う？&:2#\x02かう&やめとく#\x00>")
    ...
    if world.settings.enhance_map_compass:
        reward_list = {'Kokiri Emerald':   "##\x02コキリのヒスイ##\x00",
                       'Goron Ruby':       "##\x01ゴロンのルビー##\x00",
                       'Zora Sapphire':    "##\x03ゾーラのサファイア##\x00",
                       'Forest Medallion': "##\x02森のメダル##\x00",
                       'Fire Medallion':   "##\x01炎のメダル##\x00",
        ...
        dungeon_list = {'Deku Tree':          ("##\x02デクの樹##\x00", 'Queen Gohma', 0x62, 0x88),
    ...
    if world.settings.text_shuffle == 'except_hints':
        if world.settings.warp_songs:
            update_warp_song_text_jp(messages, world)
        reproduce_messages_jp(messages)
        shuffled = shuffle_messages_jp(messages, except_hints=True)
        write_messages(rom, shuffle = True, shuffle_group = shuffled)
    elif world.settings.text_shuffle == 'complete':
        if world.settings.warp_songs:
            update_warp_song_text_jp(messages, world)
        reproduce_messages_jp(messages)
        shuffled = shuffle_messages_jp(messages, except_hints=False)
        write_messages(rom, shuffle = True, shuffle_group = shuffled)
    elif world.settings.text_shuffle == 'none':
        if world.settings.warp_songs:
            update_warp_song_text_jp(messages, world)
        reproduce_messages_jp(messages)
        write_messages(rom, shuffle = False)
    ...
    # Enable MM-like Bunny Hood behavior (1.5ﾃ・speed)
